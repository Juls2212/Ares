...
One Electron Main scheduler owns local reminder and event-start delivery. When Ares is running, it performs one bounded catch-up check after Electron is ready, then checks every 30 seconds. It conditionally claims each due `PENDING` reminder as `TRIGGERED` with `deliveredAt` before showing one Electron desktop notification. It also considers at most 20 events ordered by start instant and ID, only when their exact `startsAt` is within `[now - 5 minutes, now]`; it atomically inserts a durable `(event_id, scheduled_at)` delivery record only when the event still has that same start instant. These durable claims provide at-most-once delivery across scheduler ticks, restarts, and concurrent Main instances. A short restart may catch up a just-missed event once, while older historical events are ignored. If notification display fails after either successful claim, Ares records a controlled Main-only technical failure and does not reset or retry, avoiding duplicate alerts. Delivery stops during Main shutdown before the database pool is closed. There is no renderer notification API, web notification, remote push, or persistent microphone processing. Ares cannot deliver notifications while its process is closed.
